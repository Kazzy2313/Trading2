...
def calculate_boursorama_linebreak(close_prices, reversal=2):
    """
    Calcul LineBreak selon l'implémentation Boursorama
    ...
    bricks = []
    # Première brique basée sur les deux premiers cours
    bricks.append({
    ...
    for i in range(2, len(close_prices)):
        ...
        # RÈGLE BOURSORAMA : Vérifier d'abord les inversions
        reversal_occurred = False
        
        if last_brick['type'] == 'green':
            # Inversion si on casse le bas des 2 dernières briques vertes
            ...
            # Inversion si on casse le haut des 2 dernières briques rouges
            if len(bricks) >= reversal:
...
def get_clean_data(symbol, period="3mo"):
    ...
    try:
        ...
        # Ignorer le jour en cours si marché ouvert
...
def detect_boursorama_signal(df):
    """Détecte les signaux selon Boursorama avec reversal=2 FIXE"""
...
def create_linebreak_chart(df, bricks, title):
    ...
    # Prendre les 8-10 dernières briques (comme Boursorama)
    display_bricks = bricks[-10:] if len(bricks) > 10 else bricks
    
    fig = go.Figure()
    
    # Récupérer les dates correspondantes aux briques
    ...
    for i, (brick, date) in enumerate(zip(display_bricks, dates)):
        ...
        # Ligne médiane (caractéristique Boursorama)
        fig.add_trace(go.Scatter(
            x=[i-0.4, i+0.4],
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            title="Dates (dernières briques à droite)"
        ),
...
CAC40 = [
    ...
    ("EL.PA","EssilorLuxottica"), ("RMS.PA","Hermès"), ("KER.PA","Kering"),
    ("LR.PA","Legrand"), ("MC.PA","LVMH"), ("ORA.PA","Orange"), ("RI.PA","Pernod Ricard"),
    ("PUB.PA","Publicis"), ("RNO.PA","Renault"), ("GLE.PA","Société Générale"),
...
SBF120 = CAC40 + [
    ...
    ("LI.PA", "Klepierre"), ("OR.PA", "L'Oréal"), ("MMB.PA", "Lagardère"),
    ...
    ("SK.PA", "SEB"), ("GLE.PA", "Société Générale"), ("SOI.PA", "Soitec"),
    ("TEP.PA", "Teleperformance"), ("TFI.PA", "TF1"), ("FR.PA", "Valéo"),
...
def toggle_favori(symbol, name):
    ...
    if favori_item in st.session_state.favoris:
        ...
        st.success(f"❌ {name} retiré des favoris")
    else:
        st.session_state.favoris.append(favori_item)
        st.success(f"⭐ {name} ajouté aux favoris")
    
    # Forcer le rerun pour mettre à jour l'interface
    st.rerun()

# ---------------------------------------------------------------------
# SCANNER TRADING (TOUS LES INDICES) - GARDÉ TEL QUEL
# ---------------------------------------------------------------------
def run_scanner():
    st.markdown("### 🔍 Scanner Trading")
    ...
    st.info("""
    **🎯 Règle LineBreak :** Détection des actions où les **2 dernières briques LineBreak alternent** 
    - 🔴 → 🟢 **ROUGE puis VERT** = Tendance haussière
    - 🟢 → 🔴 **VERT puis ROUGE** = Tendance baissière
    - **Période :** 3 mois • **Reversal :** 2 briques
    """)
    
    # Options indicateurs seulement
    st.sidebar.markdown("### 📊 Options indicateurs")
    ...
    # Sélection de l'indice dans la sidebar
    st.sidebar.markdown("### 📈 Indice à scanner")
    ...
    if st.sidebar.button("🚀 Lancer le scan", key="scan_trading") or not st.session_state.scan_done:
...
def perform_scan(selected_index):
    """Effectue le scan et stocke les résultats"""
    ...
    for i, (symbol, name) in enumerate(stocks_list):
        status_text.text(f"🔍 Analyse de {name}...")
        progress_bar.progress((i + 1) / len(stocks_list))
        
        try:
            # Période FIXE 3mo pour LineBreak
...
def display_scan_results(hits, show_bollinger, show_ema, show_macd, auto_display, selected_index):
    """Affiche les résultats du scan avec interactivité"""
    if hits:
        st.success(f"**{len(hits)} signal(s) détecté(s) dans le {selected_index}**")
        ...
        for symbol, name, df, bricks in hits:
            ...
            if last_two and last_two[0]['type'] == 'red' and last_two[1]['type'] == 'green':
                signal_type = "🔴 → 🟢 ROUGE → VERT"
                signal_emoji = "🔴🟢"
                signal_direction = "HAUSSE"
            else:
                signal_type = "🟢 → 🔴 VERT → ROUGE" 
                signal_emoji = "🟢🔴"
            ...
            with st.expander(f"{signal_emoji} {name} ({symbol}) - {signal_type} - {signal_direction}", expanded=auto_display):
                ...
                with col_btn1:
                    ...
                    favori_text = "❌ Retirer des favoris" if is_favori else "⭐ Ajouter aux favoris"
                    if st.button(favori_text, key=f"fav_{symbol}"):
                        toggle_favori(symbol, name)
                
                # Métriques - CORRIGÉ : SUPPRIMER key=
                ...
                col3.metric("Dernière MAJ", df.index[-1].strftime("%d/%m/%Y"))
                col4.metric("Signal", signal_direction)
                
                # Graphiques avec option de période pour le graphique complet seulement
                st.markdown("#### 📊 Graphiques")
                
                # Sélecteur de période pour le graphique complet avec interactivité
                ...
                # Récupérer données pour la période choisie
                df_chart = get_clean_data(symbol, chart_period)
                
                tab1, tab2 = st.tabs(["📈 Graphique Complet", "🧱 LineBreak (3 mois)"])
                ...
                with tab1:
                    if df_chart is not None:
                        ...
                        st.error("Données non disponibles pour cette période")
                ...
                with tab2:
                    ...
                    if lb_chart:
                        ...
                        st.warning("Données LineBreak insuffisantes")
    
    else:
        st.info(f"🤖 Aucune alternance détectée dans le {selected_index} pour le moment.")
        st.caption("💡 _Les signaux LineBreak sont rares - c'est normal d'en avoir peu_")

# ---------------------------------------------------------------------
# ANALYSE INDIVIDUELLE - COMPLÈTEMENT RECODÉE
# ---------------------------------------------------------------------
def run_individual_analysis():
    ...
    selection_mode = st.radio(
        "Mode de sélection:",
        ["📋 Liste prédéfinie", "🔍 Recherche libre"],
        horizontal=True,
        key="selection_mode"
    )
    
    if selection_mode == "📋 Liste prédéfinie":
        ...
        st.info("💡 **Recherche libre** : Entrez n'importe quel symbole Yahoo Finance (ex: AIR.PA, AAPL, MSFT, etc.)")
        ...
        if custom_symbol:
            ...
            name = symbol  # Par défaut, utiliser le symbole comme nom
            
            # Essayer de récupérer le nom complet via yfinance
            try:
                ...
                pass  # Garder le symbole comme nom si échec
        else:
            st.warning("Veuillez entrer un symbole Yahoo Finance")
            return
    
    # Stocker la dernière action analysée
    st.session_state.last_analyzed = (symbol, name)
    
    # Afficher l'analyse
    display_individual_analysis(symbol, name)

def display_individual_analysis(symbol, name):
    """Affiche l'analyse individuelle d'une action - CORRIGÉE"""
    ...
    st.sidebar.markdown("### ⏰ Période graphique")
    ...
    # Données pour LineBreak (3mo FIXE)
    df_lb = get_clean_data(symbol, "3mo")
    # Données pour graphique complet (période choisie)
    df_chart = get_clean_data(symbol, chart_period)
    ...
    with fav_col1:
        if st.button(favori_text, key=f"fav_indiv_{symbol}"):
            toggle_favori(symbol, name)
    ...
    col1.metric("Prix", f"{current_price:.2f}", f"{change_pct:+.2f}%")
    col2.metric("Variation", f"{change:+.2f}")
    col3.metric("Dernière MAJ", df_lb.index[-1].strftime("%d/%m/%Y"))
    ...
    if signal_detected and bricks and len(bricks) >= 2:
        ...
        if last_two[0]['type'] == 'red' and last_two[1]['type'] == 'green':
            ...
            signal_direction = "BAISSE"
        col4.metric("Signal", signal_direction)
        st.success(f"🎯 **{signal_type} détecté !** - Signal {signal_direction}")
    else:
        col4.metric("Signal", "Aucun")
        st.info("🤖 Aucune alternance détectée")
    ...
    with tab2:
        if bricks:
            ...
            if lb_chart:
                st.plotly_chart(lb_chart, use_container_width=True)
            else:
                st.warning("Données LineBreak insuffisantes")

# ---------------------------------------------------------------------
# FONCTIONS FAVORIS (GARDÉES TELLES QUELLES)
# ---------------------------------------------------------------------
def run_favoris():
    st.markdown("### ⭐ Scanner Favoris")
    
    init_favoris()
    
    if not st.session_state.favoris:
        st.info("🌟 Aucun favori ajouté. Utilisez le scanner Trading pour ajouter des actions à vos favoris.")
        return
    
    st.info(f"**📊 {len(st.session_state.favoris)} action(s) dans les favoris**")
    ...
    mode_favoris = st.radio(
        "Mode d'affichage:",
        ["🔍 Scanner les favoris", "⭐ Tous mes favoris"],
        horizontal=True,
        key="mode_favoris"
    )
    
    if mode_favoris == "🔍 Scanner les favoris":
...
def scan_favoris(show_bollinger, show_ema, show_macd, auto_display):
    """Scan des favoris pour détecter les signaux LineBreak"""
    if not st.session_state.favoris:
        return
    ...
    for i, (symbol, name) in enumerate(st.session_state.favoris):
        status_text.text(f"🔍 Scan de {name}...")
    ...
    if hits:
        st.success(f"**🎯 {len(hits)} signal(s) détecté(s) parmi vos favoris**")
        ...
        for symbol, name, df, bricks in hits:
            ...
            with st.expander(f"{signal_emoji} {name} ({symbol}) - {signal_type} - {signal_direction}", expanded=auto_display):
                ...
                if st.button("❌ Retirer des favoris", key=f"remove_fav_{symbol}"):
                    toggle_favori(symbol, name)
                ...
                # Graphiques avec option de période
                st.markdown("#### 📊 Graphiques")
                
                # Sélecteur de période pour le graphique complet
                chart_period = st.selectbox(
                    f"Période pour {name}",
                    ["1mo", "3mo", "6mo", "1y"],
                    index=1,
        ...
        st.info("🤖 Aucun signal détecté parmi vos favoris pour le moment.")
        st.caption("💡 _Les signaux LineBreak sont rares - surveillez régulièrement vos favoris_")
...
def display_all_favoris(show_bollinger, show_ema, show_macd, auto_display):
    ...
    for i, (symbol, name) in enumerate(st.session_state.favoris):
        status_text.text(f"📊 Chargement de {name}...")
        progress_bar.progress((i + 1) / len(st.session_state.favoris))
...
def main():
    ...
    app_mode = st.sidebar.selectbox(
        "Choisir le mode:",
        ["Scanner Trading", "Analyse Individuelle", "⭐ Favoris", "Analyse Technique"]
    )
    
    # Initialisation des favoris
    init_favoris()
    
    # Bouton d'actualisation général dans la sidebar
    st.sidebar.markdown("---")
    if st.sidebar.button("🔄 Actualiser toutes les données", type="secondary"):
        st.cache_data.clear()
        # Réinitialiser tous les états sauf les favoris et last_analyzed
    ...
    if app_mode == "Scanner Trading":
        ...
        **Combinaison de signaux :**
        - ✅ **LineBreak rouge→vert** + ✅ **Cours touche bande Bollinger basse** + ➡️ **Courbe tendance neutre/hausse** = 📈 Signal haussier renforcé
        - ✅ **LineBreak vert→rouge** + ✅ **Cours touche bande Bollinger haute** + ➡️ **Courbe tendance neutre/baisse** = 📉 Signal baissier renforcé

        ### 📈 Indicateurs Techniques Intégrés

        **Bandes de Bollinger**
        - **Calcul :** Moyenne mobile 20 jours + 2 écarts-types
        - **Bande supérieure =** SMA20 + (2 × écart-type)
        - **Bande inférieure =** SMA20 - (2 × écart-type)
        - **Utilité :** Repérer les niveaux de surachat/survente

        **EMA 20 (courbe de tendance)**
        - **Calcul :** Moyenne mobile exponentielle sur 20 jours
        - **Formule :** EMA = (Prix × α) + (EMA_précédent × (1-α)) où α = 2/(20+1)
        - **Interprétation :** 
          - Courbe en hausse = tendance haussière
          - Courbe en baisse = tendance baissière  
        ...
        **MACD**
        ...
        - **Interprétation :**
          - Au-dessus de 0 = tendance haussière
          - En-dessous de 0 = tendance baissière
          - Ligne MACD croise ligne de signal = changement de momentum

        ### 🔄 Fonctionnalités Interactives
        - Scan automatique du CAC40, SBF120 et Dow Jones
        - Système de favoris avec analyse complète
        - Graphiques interactifs avec dates
        - Actualisation manuelle des données
        - Indicateurs personnalisables

        *📈 Données Yahoo Finance • 🎯 Algorithmes professionnels • ⚠️ À titre éducatif uniquement*

        **⚠️ AVERTISSEMENT :** Ce scanner est un outil d'aide à la décision. 
